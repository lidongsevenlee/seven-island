...
**Seven Island** is a macOS notch enhancement app — it turns the MacBook notch into a dynamic hub: music controls, calendar, clipboard history, file shelf with AirDrop, HUD replacement (volume/brightness), and developer tool monitoring (Codex/Codex sessions). Written in SwiftUI for macOS 14+.
...
**Sandbox requirement:** When running this script from Codex's Bash tool, you must use `dangerouslyDisableSandbox: true` — the default sandbox blocks SPM package resolution during the xcodebuild step.
...
| `sevenIslandFeatureNotchHeight` | 360 (1.5×) | Clipboard, VS Code, Codex/Codex views (max open height) |
...
**Closed state** renders live activities directly in NotchLayout (music visualizer, Codex/Codex activity icons, battery notifications, face animation). Open/closed transitions are animated springs.
...
| Codex status | `Models/ClaudeStatusSnapshot.swift` | `Services/ClaudeStatusService.swift` | `Views/ClaudeStatusView.swift` |
...
**Dynamic notch height:** For Codex and Codex views, the ContentView does NOT set a fixed notch height. Each view's `.task` modifier computes its own height based on session count (`count * 38 + chrome`) and calls `vm.setOpenNotchHeight()`. ContentView keeps the current height (`vm.notchSize.height`) when switching to these views. This avoids animation conflicts between ContentView's 0.35s transition and the subview's 0.24s height update.
...
- **Codex:** reads `~/.Codex/sessions/*.json` + `~/.Codex/projects/{encoded-cwd}/{sessionId}.jsonl`. CWD encoding: same `-` replacement. Session `startedAt` is a millisecond Unix timestamp (Int). JSONL tool_use blocks are inside `message.content[]` (not top-level). Idle threshold: 30 seconds
