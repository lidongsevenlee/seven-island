...
struct ContentView: View {
    ...
    func CodexLiveActivity() -> some View {
        ...
        HStack {
            PixelCodexIcon(size: statusSize, isWorking: activity?.state == .working)
...
private struct PixelCodexIcon: View {
    let size: CGFloat
    let isWorking: Bool
    ...
    var body: some View {
        ZStack {
            ...
            ZStack(alignment: .topLeading) {
                ...
                block(x: 4, y: isAnimating ? 2 : 3)
                block(x: 3, y: isAnimating ? 3 : 4)
                block(x: 5, y: isAnimating ? 3 : 4)
                block(x: 4, y: isAnimating ? 4 : 5)

                if isWorking {
                    block(x: isAnimating ? 7 : 6, y: 1)
                    block(x: isAnimating ? 8 : 7, y: 0)
                }
            }
            .frame(width: unit * 9, height: unit * 9, alignment: .topLeading)
            .animation(.easeInOut(duration: 0.48).repeatForever(autoreverses: true), value: isAnimating)
        }
        .frame(width: size, height: size)
        .onAppear {
            isAnimating = true
    ...
    private func block(x: CGFloat, y: CGFloat, w: CGFloat = 1, h: CGFloat = 1) -> some View {
        Rectangle()
            .fill(Color.green)
            .frame(width: unit * w, height: unit * h)
